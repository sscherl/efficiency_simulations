...
from classes.toolkit import Chip, File, LHCParameters, Helper, AstroPixParameters
...
class DataTransformation:
	...
	def __init__(self, data, sensor, events, rate, layers, quadrants, secondaries, clusters, tot, extra, repetition, save_plots):
		...
		self.data_width 		= config['data']['width']
	...
	###### Merge data from all daisy chained chips ##################

	# def merge_data(self, chip_id):

	# 	if chip_id == config['chip']['first']:
	# 		self.df_all_chips = self.df
	# 	else:
	# 		self.df_all_chips.append(self.df)
	...
	def set_time_between_hits(self):
		...
		self.df['ToH'] = ((self.df['Event'] - 1) * self.astro.event_length_ns) + self.df['ToF']
